...
def handler(ctx, data: io.BytesIO = None):
    try:
        ...
        if action == "create":
            ...
            return read_user(ctx, payload)
        elif action == "update":
            return update_user(ctx, payload)
        elif action == "delete":
            return delete_user(ctx, payload)
        else:
            raise ValueError("Invalid action: {}".format(action))

    except Exception as ex:
        print('ERROR: Invalid payload or action', ex, flush=True)
        raise
